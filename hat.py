...
#
# Code modified from: https://github.com/XPixelGroup/HAT
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, rpi, mask=None):
        ...
        # print("attn", attn.shape, relative_position_bias.shape)
...
class HAT(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)
            feat_x = self.forward_features(x)
            x = self.conv_after_body(feat_x) + x
            x = self.conv_before_upsample(x)
            x = self.upsample(x)
            # x = self.conv_last(x)
        x = x[:, :, :4*h, :4*w]
        
        # Inverse RGB Normalization
        # print(x.shape)
